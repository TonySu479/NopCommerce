...
BEGIN
	...
	/* Celebrities that filtered by keywords */
	CREATE TABLE #KeywordCelebrities
	(
	...
	BEGIN
			...
			BEGIN
				...
				BEGIN		
					...
					BEGIN
                        ...
                        --LEN excludes trailing spaces. That is why we use DATALENGTH
						--see https://docs.microsoft.com/sql/t-sql/functions/len-transact-sql?view=sqlallproducts-allversions for more details
						SET @len_keywords = DATALENGTH(@Keywords) / @len_nvarchar
		...
		--celebrity name
		SET @sql = '
		INSERT INTO #KeywordCelebrities ([CelebrityId])
		SELECT p.Id
		FROM Celebrity p with (NOLOCK)
		WHERE '
		IF @UseFullTextSearch = 1
			SET @sql = @sql + 'CONTAINS(p.[Name], @Keywords) '
		ELSE
		...
		--localized celebrity name
		SET @sql = @sql + '
		UNION
		SELECT lp.EntityId
		FROM LocalizedProperty lp with (NOLOCK)
		WHERE
			lp.LocaleKeyGroup = N''Celebrity''
			AND lp.LanguageId = ' + ISNULL(CAST(@LanguageId AS nvarchar(max)), '0') + '
			AND ( (lp.LocaleKey = N''Name'''
		IF @UseFullTextSearch = 1
			SET @sql = @sql + ' AND CONTAINS(lp.[LocaleValue], @Keywords)) '
		...
		IF @SearchCelebrityTags = 1
		BEGIN
			--celebrity tags (exact match)
			SET @sql = @sql + '
			UNION
			SELECT pptm.Celebrity_Id
			FROM Celebrity_CelebrityTag_Mapping pptm with(NOLOCK) INNER JOIN CelebrityTag pt with(NOLOCK) ON pt.Id = pptm.CelebrityTag_Id
			WHERE pt.[Name] = @OriginalKeywords '

			--localized celebrity tags
			SET @sql = @sql + '
			UNION
			SELECT pptm.Celebrity_Id
			FROM LocalizedProperty lp with (NOLOCK) INNER JOIN Celebrity_CelebrityTag_Mapping pptm with(NOLOCK) ON lp.EntityId = pptm.CelebrityTag_Id
			WHERE
				lp.LocaleKeyGroup = N''CelebrityTag''
				AND lp.LanguageId = ' + ISNULL(CAST(@LanguageId AS nvarchar(max)), '0') + '
				AND lp.LocaleKey = N''Name''
				AND lp.[LocaleValue] = @OriginalKeywords '
		END
	...
	SELECT p.Id
	FROM
		Celebrity p with (NOLOCK)'
	
	IF ISNULL(@CelebrityTagId, 0) != 0
	BEGIN
		SET @sql = @sql + '
		INNER JOIN Celebrity_CelebrityTag_Mapping pptm with (NOLOCK)
			ON p.Id = pptm.Celebrity_Id'
	END
	...
	BEGIN
		SET @sql = @sql + '
		JOIN #KeywordCelebrities kp
			ON  p.Id = kp.CelebrityId'
	END
	
	SET @sql = @sql
	
	--filter by celebrity tag
	IF ISNULL(@CelebrityTagId, 0) != 0
	BEGIN
		SET @sql = @sql + '
    ...
    SET @sql = '
    INSERT INTO #DisplayOrderTmp ([CelebrityId])' + @sql

	--PRINT (@sql)
	EXEC sp_executesql @sql

	DROP TABLE #KeywordCelebrities
	...
	INSERT INTO #PageIndex ([CelebrityId])
	SELECT CelebrityId
	FROM #DisplayOrderTmp
	...
	FROM
		#PageIndex [pi]
		INNER JOIN Celebrity p with (NOLOCK) on p.Id = [pi].[CelebrityId]
	WHERE
		[pi].IndexId > @PageLowerBound AND 
		[pi].IndexId < @PageUpperBound
	ORDER BY
