...
namespace Nop.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        /// Gets pictures by product identifier
        /// </summary>
        /// <param name="celebrityId">Product identifier</param>
        /// <param name="recordsToReturn">Number of records to return. 0 if you want to get all items</param>
        /// <returns>Pictures</returns>
        public virtual IList<Picture> GetPicturesByCelebrityId(int celebritytId, int recordsToReturn = 0)
        {
            if (celebritytId == 0)
                return new List<Picture>();

            var query = from p in _pictureRepository.Table
                        join pp in _celebrityPictureRepository.Table on p.Id equals pp.PictureId
                        orderby pp.DisplayOrder, pp.Id
                        where pp.CelebrityId == celebritytId
                        select p;
